...
function matchesCatalog(c) {
  switch (catalogFilter) {
    case 'Седаны': return c.body === 'Седан';
    case 'Кроссоверы': return c.body === 'Кроссовер';
    case 'Электро': return c.type === 'Электро';
    case 'Гибриды': return c.type === 'Гибрид';
    default: return true;
  }
}
function screenCatalog() {
  const list = CARS.filter(matchesCatalog);
  const chips = ['Все', 'Седаны', 'Кроссоверы', 'Электро', 'Гибриды'];
  return '<div class="screen"><div class="scroll pad-nav">' +
    ...
    '<div class="card" style="flex:1;display:flex;align-items:center;gap:8px;padding:12px 14px;color:var(--tx3);">' + icon('search', 17) + '<span style="font-size:13px;">Марка, модель...</span></div>' +
    '<div class="icon-btn" style="width:46px;height:46px;background:var(--gold-soft);border-color:var(--gold);">' + icon('sliders', 18, 1.9) + '</div></div>' +
    '<div class="scroll" style="display:flex;gap:8px;overflow-x:auto;padding:16px 20px 4px;">' +
    chips.map(ch => '<span class="' + (catalogFilter === ch ? 'chip-on' : 'chip') + '" style="flex:none;cursor:pointer;" data-filter="' + ch + '">' + ch + '</span>').join('') + '</div>' +
    '<div class="container" style="padding:14px 20px 4px;font-size:12px;color:var(--tx3);">Найдено ' + list.length + (list.length === 1 ? ' вариант' : ' варианта') + '</div>' +
    '<div class="container" style="display:flex;flex-direction:column;gap:12px;padding:12px 20px 4px;">' +
    (list.length ? list.map(carRow).join('') : emptyState('Ничего не найдено', 'Попробуйте выбрать другую категорию')) + '</div>' +
...
function screenFinder() {
  ...
  return '<div class="screen"><div class="scroll pad-cta">' +
    ...
    '<div style="display:flex;align-items:center;gap:8px;color:var(--gold);">' + icon('star', 17) + '<div style="font-size:13.5px;font-weight:700;">Найдено ' + results.length + (results.length === 1 ? ' автомобиль' : ' автомобиля') + '</div></div>' +
...
function screenCalculator() {
  ...
  return '<div class="screen"><div class="scroll pad-cta">' +
    ...
    '<div class="container" style="padding:6px 20px 0;font-size:12.5px;color:var(--tx2);line-height:1.4;">Ориентировочная стоимость доставки автомобиля из Китая под ключ.</div>' +
    '<div class="container" style="padding:0 20px;">' +
    '<div class="card" style="margin:20px 0 0;padding:12px;display:flex;align-items:center;gap:12px;cursor:pointer;" data-calc-next>' +
    ...
    '<div style="display:flex;align-items:center;gap:4px;color:var(--gold);font-size:12px;font-weight:600;">Изменить' + icon('chevronRight', 14, 2) + '</div></div>' +
...
function screenServices() {
  const services = [
    ['search', 'Подбор автомобиля', 'Найдём модель под бюджет и задачи, с фото и видео с площадки в Китае.'],
    ['shield', 'Проверка и аудит', 'Технический осмотр, история пробега и юридическая чистота перед покупкой.'],
    ['truck', 'Доставка и растаможка', 'Полное сопровождение перевозки и таможенного оформления в РФ.'],
    ['doc', 'Документы и регистрация', 'Оформим ЭПТС, поставим на учёт в ГИБДД, поможем со страховкой.']
  ];
  return '<div class="screen"><div class="scroll" style="padding-bottom:40px;">' +
    ...
    services.map(s => '<div class="card" style="padding:16px;display:flex;gap:14px;align-items:flex-start;">' +
...
function screenFavorites() {
  ...
  return '<div class="screen"><div class="scroll pad-nav">' +
    ...
    '<div class="subtitle">' + list.length + (list.length === 1 ? ' сохранённый автомобиль' : ' сохранённых автомобиля') + '</div></div>' +
...
function screenProfile() {
  function mi(iconName, label, count) {
    return '<div style="display:flex;align-items:center;gap:12px;padding:14px 4px;border-bottom:1px solid var(--line);">' +
  ...
  return '<div class="screen"><div class="scroll pad-nav">' +
    ...
    '<div class="icon-btn">' + icon('edit', 16) + '</div></div>' +
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:14px 0 0;">' +
    '<div class="card" style="padding:13px;text-align:center;"><div class="h" style="font-size:19px;font-weight:800;color:var(--gold);">2</div><div style="font-size:11px;color:var(--tx3);margin-top:2px;">активные заявки</div></div>' +
    '<div class="card" style="padding:13px;text-align:center;"><div class="h" style="font-size:19px;font-weight:800;color:var(--gold);">1</div><div style="font-size:11px;color:var(--tx3);margin-top:2px;">заказ в пути</div></div></div>' +
    '<div class="card" style="margin:20px 0 0;padding:4px 14px;">' +
    mi('doc', 'Мои заявки', 2) + mi('truck', 'Мои заказы', 1) + mi('clock', 'История заказов') + mi('heart', 'Избранное', [...favorites].length) +
    mi('wallet', 'Способы оплаты') + mi('grid', 'Услуги') + mi('home', 'Как мы работаем') + mi('bell', 'Контакты и поддержка') + '</div>' +
    '<div style="margin:14px 0 30px;display:flex;align-items:center;gap:10px;padding:14px 4px;color:var(--danger);cursor:pointer;">' +
...
function screenOrder() {
  ...
  return '<div class="screen"><div class="scroll" style="padding-bottom:40px;">' +
    ...
    '<div class="icon-btn" style="background:var(--gold-soft);border-color:var(--gold);">' + icon('telegram', 16) + '</div></div>' +
    '</div></div></div>';
}
...
document.addEventListener('click', function (e) {
  ...
  if (filterEl) { catalogFilter = filterEl.dataset.filter; render(); return; }

  const calcEl = e.target.closest('[data-calc-next]');
  if (calcEl) { calcCarIndex = (calcCarIndex + 1) % CARS.length; render(); return; }
